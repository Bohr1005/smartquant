...
public:
    ...
    void close_long()
    {
        ...
        auto spread = spot_->last_price() - future_->last_price();
        auto spread_ma = indicator_.mean();
        auto close_signal = spread <= spread_ma;

        if (close_signal && !close_flag_)
        {
            close_flag_ = true;
        }
    ...
    void on_event(const Event& event) override
    {
        ...
        if ((future_->datetime().time() > Time(9,31,0) && future_->datetime().time() < Time(11,29,0)) || (future_->datetime().time() > Time(13,1,0) && future_->datetime().time() < Time(14,55,0)))
        {
