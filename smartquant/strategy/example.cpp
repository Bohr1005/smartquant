...
public:
    ...
    void on_trade(const TradeData& trade)
    {
        if (future_->net_pos() != 0)
        {
            ...
            if (!time_)
            {
                time_ = std::chrono::duration_cast<std::chrono::milliseconds>(now).count();
            }
        }
        else
        {
    ...
    void close_long()
    {
        ...
        {
            auto now = std::chrono::steady_clock::now().time_since_epoch();
            auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(now).count() - time_;
            
            if (duration >= wait_ && !close_flag_)
            {
                close_flag_ = true;
                close_long_.kill();
            }
            ...
            {
                if (close_long_.price() > future_->bid_1_price())
                {
                    close_long_.kill();
                }
            }
        ...
        else
        {
            ...
            {
                auto our_price = future_->bid_1_price();
                close_long_.send(our_price,future_->net_pos());
            }
        }
    }
    ...
    void close_short()
    {
        ...
        if (close_short_.is_active())
        {
            ...
            {
                if (close_short_.price() < future_->ask_1_price())
                {
                    close_short_.kill();
                }
            }
        ...
        else
        {
            ...
            {
                auto our_price = future_->ask_1_price();
                close_short_.send(our_price,-future_->net_pos());
            }
        }
    }
